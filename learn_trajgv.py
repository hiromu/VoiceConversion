...
if __name__ == '__main__':
    ...
    for i in xrange(len(source_list)):
        ...
        if gv is None:
            gv = target_mfcc.var(0)
        else:
            gv = numpy.vstack([gv, target_mfcc.var(0)])

    gmm = sklearn.mixture.GMM(n_components = K, covariance_type = 'full')
    gmm.fit(learn_data)

    gv_gmm = sklearn.mixture.GMM(n_components = 1, covariance_type = 'full')
    gv_gmm.fit(gv)

    gmmmap = TrajectoryGMMMap(gmm, gv_gmm)
