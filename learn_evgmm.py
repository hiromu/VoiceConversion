...
if __name__ == '__main__':
    ...
    for i in xrange(len(target_list)):
        ...
        for j in xrange(len(target_list[i])):
            ...
            cache_path = os.path.join(sys.argv[3], '%s_%s.dtw' % tuple(map(lambda x: re.sub('[./]', '_', re.sub('^[./]*', '', x)), [source_list[j], target_list[i][j]])))
            if os.path.exists(cache_path):
                dtw = pickle.load(open(cache_path))
            else:
                dtw = DTW(source_mfcc[j], target_mfcc, window = abs(source.SPEC.shape[0] - target.SPEC.shape[0]) * 2)
